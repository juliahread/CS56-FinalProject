...
Controls::Controls(Background *controlsbg, SDL_Renderer *renderer)
  ...
  // load intro text
  m_intro.push_back(
      Text("It's the year 2200, and the Claremont Colleges", 42, {35, 20}));
  m_intro.push_back(
      Text("have successfully migrated to space...", 42, {35, 70}));
  m_intro.push_back(
      Text("But over-enrollment in CS 5 at HMC has caused a", 42, {35, 120}));
  m_intro.push_back(
      Text("singularity to form, and the Claremont Colleges", 42, {35, 170}));
  m_intro.push_back(
      Text("have been destroyed! You must reach Campus", 42, {35, 220}));
  m_intro.push_back(
      Text("Safety in order to survive. Good luck!", 42, {35, 270}));

  // load controls text
  m_controls.push_back(
      Text("Left Click - shoot the grappling hook", 36, {200, 350}));
  m_controls.push_back(
      Text("Right Click - detach the grappling hook", 36, {200, 400}));
