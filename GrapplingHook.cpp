...
GrapplingHook::GrapplingHook(Player *shooter, Map *map, SDL_Renderer *renderer)
    : m_fired(false),
      m_spin(None),
      m_was_spinning(false),
      m_distance_sq(0),
      m_shooter(shooter),
      m_map(map) {
  m_sprsheet = new SpriteSheet("images/grapplingstart.png", renderer, 1, 0);
...
void GrapplingHook::shoot(SDL_Point click) {
  ...
  const SDL_Point *potential_anchor =
      m_map->get_grappling_point_list()->findClosestGrapplePoint(click);
  Vec2D player_loc = m_shooter->get_pos();
  SDL_Point *intersection = m_map->get_obstacle_list()->intersectLine(
      player_loc.toSDL_Point(), *potential_anchor);
  if (intersection == nullptr) {
    // No collision, so valid grapple
    m_wrap_points.push_back({*potential_anchor, None});
    std::cout << "anchor x: " << potential_anchor->x
              << " anchor y: " << potential_anchor->y << std::endl;
    m_distance_sq =
        std::pow(std::abs(potential_anchor->x - player_loc.m_x), 2) +
...
void GrapplingHook::render(SDL_Renderer *renderer) const {
  if (m_fired) {
    ...
    m_sprsheet->renderSpriteRotatedCentered(player_loc.x, player_loc.y,
                                            renderer, 1, -angle);
  }
}
...
void GrapplingHook::update() {
  if (m_fired) {
    ...
    SDL_Point *intersection =
        m_map->get_obstacle_list()->intersectLine(shooter_pos, last_anchor);
    if (intersection != nullptr) {
      m_wrap_points.push_back({*intersection, m_spin});
      std::cout << "intersection pushed" << intersection->x << " "
    ...
    if (m_wrap_points.size() > 1) {
      ...
      float z = current_line.cross_z(prev_line) /
                (current_line.get_length() * prev_line.get_length());
...
bool GrapplingHook::check_fired() const { return m_fired; }
...
float GrapplingHook::dist_sq_from_last_anchor() const {
  SDL_Point last_anchor = *get_last_anchor();
  SDL_Point player_loc = m_shooter->get_pos().toSDL_Point();
  return std::pow(std::abs(last_anchor.x - player_loc.x), 2) +
         std::pow(std::abs(last_anchor.y - player_loc.y), 2);
}
...
bool GrapplingHook::is_spinning() const {
  if (m_fired) {
    ...
    return (dist_sq_from_last_anchor() >= m_distance_sq) &&
           (current_line.dot(m_shooter->get_vel()) <= 0);
  } else {
    return false;
  }
}

Spin GrapplingHook::get_spin() const { return m_spin; }

void GrapplingHook::set_spin(Spin spin) { m_spin = spin; }
...
void GrapplingHook::update_player_vel() {
  ...
  Vec2D anchor_to_player(player_loc.m_x - sdl_anchor_loc->x,
                         player_loc.m_y - sdl_anchor_loc->y);
  ...
  if (not m_was_spinning and is_spinning()) {
    cos_theta = abs(
        anchor_to_player.dot(m_shooter->get_vel()) /
        (anchor_to_player.get_length() * m_shooter->get_vel().get_length()));
    if (isnan(cos_theta)) {
