...
void Scores::update_text() {
  ...
  for (auto highscore : m_scores) {
    m_scores_text.push_back(
        Text(highscore.name, 50, {name_x, start_y + (line * line_height)}));
    m_scores_text.push_back(Text(std::to_string(highscore.score), 50,
                                 {score_x, start_y + (line++ * line_height)}));
  }
}
