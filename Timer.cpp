...
Timer::Timer()
    : m_text_timer(Text("Time: " + std::to_string(m_time), 50, {950, 20})) {}

Timer::~Timer() {}

void Timer::start() { std::time(&start_time); }

void Timer::stop() { std::time(&stop_time); }

void Timer::set(int new_time) { m_time = new_time; }
